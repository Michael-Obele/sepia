...
- **Shared data layer**: `@sepia/shared` exports the Drizzle client (`db()`), the schema, and all data-layer functions (`createEntity`, `listMemories`, `search`, `traverseGraph`, `pruneMemories`, `stats`, etc.). Reuse these — do not re-implement DB access in the dashboard.
