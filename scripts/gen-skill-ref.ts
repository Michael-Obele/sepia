...
const sections: Record<string, string[]> = {
  ...
  manage_memory: [
    ...
    `- \`briefing\`: **the session-start read — call it once, before any work.** Returns the CORE standing rules: everything tagged \`${ALWAYS_TAG}\` plus every instruction/preference at importance >= ${CORE_IMPORTANCE}, priority-ordered, each rule compacted to ${BRIEFING_ITEM_CHARS} chars (full text stays reachable via \`get\` on the id). \`detail\`? = \`core\` (default) | \`all\` — \`all\` also returns the growing tail of situational rules, and is the right call immediately BEFORE an install, build, deploy, deletion, or infra change — and on the FIRST session-start call when the context window is large (~1M tokens): the whole set is ~10k tokens (≈1% of that window), read once per session, priority-ordered with core first. \`max_chars\`? (default ${BRIEFING_CHARS_DEFAULT}, max ${BRIEFING_CHARS_MAX} = FETCH_MAX rows × ITEM_CHARS, the physical maximum) is the budget, and applies to \`detail="all"\` only. Returns \`{ detail, count, core_count, other_standing, truncated, omitted, max_chars?, memories }\`. Core rules are NEVER dropped for budget; \`truncated\` + an exact \`omitted\` state what was left out of what you ASKED for, and \`other_standing\` is the tail you did not ask for. A standing constraint cannot be found by keyword search, which is why this read is unconditional.`,
